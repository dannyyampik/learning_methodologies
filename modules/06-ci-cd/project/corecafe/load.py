...
v2 (Module 04's renovation): sales are loaded as ATOMIC PARTITION
OVERWRITES — the partition (one day) is the unit of work. Re-runs,
corrected re-deliveries, and backfills are all the same safe operation.
"""
...
def overwrite_sales_partition(
    con: duckdb.DuckDBPyConnection, day: date, rows: Iterable[Sale]
) -> int:
    """Atomically replace one day's rows in `sales`.

    All-or-nothing: on any error the partition is left exactly as it was.
    """
